...
test "unit: alignment helpers reject invalid alignment" {
    ...
    try testing.expectError(error.InvalidAlignment, mem.isAligned(usize, 1, 3));
}
...
test "unit: isAligned reports both states" {
    try testing.expect(try mem.isAligned(usize, 16, 8));
    try testing.expect(!try mem.isAligned(usize, 18, 8));
}

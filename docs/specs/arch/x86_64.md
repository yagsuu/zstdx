...
primitives that generic `zstdx` specs and downstream packages (`zpci`, `znvme`,
`zacpi`, kernel/firmware projects) cannot portably express through `std` or Zig
language facilities.
...
`Cpuid` exposes the raw `cpuid` instruction.

- `leaf(eax)` executes `cpuid` with `ecx = 0` and returns the resulting
  `Result`.
- `subleaf(eax, ecx)` executes `cpuid` with the supplied `ecx`.
- `maxBasicLeaf()` returns `cpuid(0).eax`.
- `maxExtendedLeaf()` returns `cpuid(0x80000000).eax`.
...
- `Segment.Cs.writeFarReturn(selector)` performs the architectural far-return
  ...
  trampoline policy.
- `Segment.FsBase.read`/`write` use `rdfsbase`/`wrfsbase` when supported, with a
  fallback through `IA32_FS_BASE` MSR access otherwise. The MSR fallback is
  privileged; the `rdfsbase`/`wrfsbase` path requires `Cr4.FSGSBASE`.
- `Segment.GsBase.read`/`write` behave analogously with `IA32_GS_BASE`.

...
view for interop. The zstdx contract is the arena contract, not the full policy
...
```text
src/mem.zig
src/mem/fixed_buffer_arena.zig
test/mem/fixed_buffer_arena_test.zig
```
...
pub const FixedBufferArena = struct {
    ...
    pub fn restore(self: *FixedBufferArena, mark: Mark) void;
...
The real implementation must leave `index` unchanged on `error.InvalidAlignment`,
...
`allocSlice(T, len)` returns `len` uninitialized `T` values stored contiguously in
arena memory aligned to `@alignOf(T)`.
...
The allocator view must not make individual frees part of the zstdx contract.
...
`FixedBufferArena` must not be a public alias for `std.heap.FixedBufferAllocator`.
The zstdx type owns its public semantics even if implementation code shares
internal algorithms with std.

## Ownership and lifetime

The arena borrows `buffer`. It never owns or frees memory.
...
- `zvm`: support parse/build scratch for profile lowering, identity digest staging,
  and bounded host-test fixtures without introducing hot-path heap allocation.
